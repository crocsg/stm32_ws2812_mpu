...
uint8_t dmp_get_fifo_packet_length (void)
{
...
/**
 *  @brief      Get one  FIFO packet from buffer
 *  If @e sensors does not contain a particular sensor, disregard the data
 *  returned to that pointer.
 *  \n @e sensors can contain a combination of the following flags:
 *  \n INV_X_GYRO, INV_Y_GYRO, INV_Z_GYRO
 ...
 *  @param[out] sensors     Mask of sensors read from FIFO.
 *  @param[out] more        Number of remaining packets.
 *  @param[in] fifo_data	data buffer
 *  @return     0 if successful.
 */
int dmp_decode_fifo(short *gyro, short *accel, long *quat,
    unsigned long *timestamp, short *sensors, unsigned char *more, uint8_t* fifo_data)
{
    //unsigned char fifo_data[MAX_PACKET_LENGTH];
    unsigned char ii = 0;
